  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-start justify-between">
        ...
        <Link href={`/proveedores/${c.proveedor_id}`} className="text-sm text-slate-900 hover:underline">
          Ver proveedor →
        </Link>
      </div>
      ...
      <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
        <div>
          <p className="text-xs text-slate-900">Estado de pago</p>
          <div className="mt-1">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {c.compra_items?.map((item: any) => (
              <tr key={item.id}>
                ...
                <td className="py-2 text-slate-900">{item.cantidad}</td>
                <td className="py-2 text-slate-900">${item.precio_unitario.toFixed(2)}</td>
        ...
        <div className="mt-4 space-y-1 border-t border-slate-200 pt-4 text-right text-sm dark:border-slate-800">
          <p className="text-slate-900">Subtotal: ${c.subtotal.toFixed(2)}</p>
          {c.iva > 0 && <p className="text-slate-900">IVA: ${c.iva.toFixed(2)}</p>}
      ...
      {c.observaciones && (
        <div className="rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
          <p className="mb-1 text-xs font-semibold text-slate-900">Observaciones</p>
          {c.observaciones}
        </div>
      )}
    </div>
